...
=head1 SYNOPSIS
 ...
 fault __x "cannot allocate {size} bytes", size => $size;
 fault "cannot allocate $size bytes";  # fails translation
...
C<enable>, C<mode>, and C<needs>.  The commands are followed by a LIST of
dispatcher NAMEs to be address.  For C<mode> see section L</Run modes>;
it requires a MODE argument before the LIST of NAMEs.  Non-existing names
will be ignored.
